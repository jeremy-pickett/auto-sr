...
**Status:** partially landed. **§8 (render modes) shipped in product release 2.2.1**
on 2026-08-20 — `activity`, `kind-stable`, the rebuilt `trails`, and the real `relief`
are in `RunView.jsx`; see `documents/architecture.md`'s Phase 10 entry and
`documents/deep-dive/06-frontend.md` §3. §8's requirements below are now a record of
what was built, not a proposal.

Sections 4, 5, and 6 — the recurrent-structure detector (REQ-19.x) — remain **specified
to contract depth and unstarted**, and are still the substantial remaining work here.
§7 (structure-aware inspection) depends on them and is likewise unbuilt; it remains at
intent depth and still needs a calibration pass before code.
