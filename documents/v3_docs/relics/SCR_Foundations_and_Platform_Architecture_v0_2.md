...
## **Foundations and Platform Architecture — v0.2**

**Status:** In review — foundational design for critique **Date:** 2026-08-20 **Scope:** Semantic Cellular Ruliology 3.x **Supersedes:** v0.1 **Purpose:** Define what the platform is, what its major components own, how humans interact with it, which principles future requirements and implementations must preserve, and how the large document tree that will follow this one stays honest.

> **This is not the implementation requirements document.** It sits above requirements. Its job is to establish the conceptual map so later requirements, code, Labs, interfaces, tests, and research papers have a stable place to belong. Roughly two hundred documents are expected downstream of this one, most of them machine-written. This document is therefore written to be *cited*, not merely read.

---

## **Revision record — v0.1 → v0.2**

Recorded rather than silently applied, following the project's existing practice: the reasoning matters more than the edits.

| Change | Nature |
| :---- | :---- |
| **World Layout subordinated to World; component count corrected to twelve** (§14–15, §34) | Consistency defect in v0.1. The component list counted "World / World Layout" as one entry while §14–15 and the ownership map treated them as two. Layout is now a property the World owns. A side effect: incoherent pairings (a relational world governed by a spatial lattice) have no slot to exist in, which retires an external reviewer's concern by construction rather than by rule. |
| **Lab vocabulary removed from the core Reader list** (§21) | Lab leak in v0.1, by the document's own §45.8 definition. The Reader examples included "compromise spread," "privilege reached," and similar security-Lab vocabulary in a section of the platform core, eleven sections after the core was forbidden from knowing what a domain controller is. Platform-neutral and Lab-illustrative examples are now separated and labeled. This matters doubly because downstream documents will be model-written, and models pattern-match on what the root document exemplifies. |
| **Demonstrated versus specified corrected** (§24) | Overstatement in v0.1. The claim that 2.x "proved" activity highlighting, trails, and relief conflated what the 2.x product demonstrably does with what the 2.2 visualization uplift specifies. The two are now distinguished, and build status is left as a repository fact this document does not assert. |
| **Foundational forks named instead of implied** (§19, §20, §40) | v0.1 contained at least four foundational forks disguised as phrasing: "exact or contractually equivalent reconstruction," a Run bound to one Plugin while every Lab example implies several interacting mechanisms, Study failure "with the required confidence" with confidence undefined, and a semantic interface that "governs all components" without an ownership home. Each is now an explicit Decision Registry entry with a status and an owning future document. Naming a fork is not deciding it. |
| **Temporal semantics stated as a Reactor capability** (§6, §18.5) | Extension prompted by external review. Adversarial and operational domains need asynchrony — stale observations, delayed effects, interleaved application — and a strictly lockstep core would exclude those Labs (§45.13). The reviewed proposal placed the capability on the wrong side of the platform's strongest boundary by letting Plugins manipulate their own temporal state. v0.2 states the correct placement: asynchrony is a declared World/Reactor capability; Plugins may *propose* future-offset effects exactly as they propose any state change; the Reactor owns the clock, the queue, quantization, and budgets. The resolution mechanics belong to a decision record, not this document. |
| **Evidence integrity principle added; steganographic provenance rejected** (§32.1) | Extension prompted by external review. The underlying question — provenance surviving lossy downstream pipelines — is legitimate. The reviewed answer, embedding a statistical watermark into Cell state matrices, is rejected on principle: in a system whose premise is sensitive dependence, modifying state data to carry metadata corrupts the experiment in order to sign it. Integrity lives in cryptography over the *record*, never in alterations to the *data*. |
| **Influence views contextualized, not censored** (§25.3) | Refinement prompted by external review. The deception risk is real: a divergence rendering can imply a flip was special when the system diverges identically from any flip. The reviewed remedy — a differentiability-based gate refusing to render — is unavailable (generated discrete-state Python has no Jacobian) and would create the invisible truth layer §21 forbids. The correct remedy is context: any specific divergence is rendered against the Run's ambient sensitivity, and uniform sensitivity is itself reported as the finding. |
| **Repairs must return meaning** (§16.5) | Adoption from external review, with a caveat attached. Requiring humans to read code diffs to verify a machine repair is a semantic leak. Repairs now owe a plain-language account of what changed mechanistically. The caveat: that account is the machine grading its own homework, so it is classified as interpretation — Reader-class, versioned, disputable — never as evidence. |
| **Cell computational ceiling** (§13.1) | Adoption from external review. §13's "useful architectural pressure" is sharpened into an enforceable Lab-fit test: state expressible as a bounded set of primitive scalars, or the Lab fit review fails. |
| **Document system given teeth** (§36) | Strengthening. v0.1 defined a documentation hierarchy but no citation identifiers, no status lifecycle, no precedence rule, and no amendment procedure. With a mostly machine-written tree, drift happens at the root, silently, one model-interpreted ambiguity at a time. §36 now defines all four, and §36.7 seeds the canonical glossary so the deliberate 2.x→3.x renames are pinned rather than re-derived. |
| **2.x corpus disposition stated** (§22.1) | Silence in v0.1. For a permanence-obsessed project, whether the existing library is carried forward, archived, or orphaned was a foundational unknown. Decided: it is carried forward as founding evidence. Migration mechanics are a registry entry (DEC-6). |
...
- **Study failure** — available evidence did not answer the hypothesis with the required confidence or coverage. What "required confidence" means is a named open decision (DEC-4), and until it is decided, documents citing this taxonomy must not invent local definitions.
...
The pressure above is now an enforceable test rather than a vibe:

> **A Cell's state must be expressible as a bounded set of primitive scalars, declared in advance.**

If a Lab cannot define its entity's state this way — if its "Cell" needs unbounded structures, embedded programs, or open-ended memory to be itself — the Lab fails the fit review (§30). This is what keeps SCR recognizably a local-mechanism instrument instead of drifting into an unconstrained general-purpose simulator, which is the over-generalization failure §45.12 asks reviewers to hunt for. The ceiling's exact numbers belong to requirements documents; the existence of a ceiling belongs here.

---

## **14\. World**

A **World** is the complete experimental environment: which Cells exist, how they are arranged, which Connections are possible, what environmental conditions apply, what is observable, and what the starting state is.

A human should be able to request a World semantically:
...
**The World owns its Layout.** v0.1 listed World and World Layout as separable components; v0.2 corrects this. How Cells are arranged and which local interactions are possible is a property *of* the experimental environment, not a sibling concern that can be mixed and matched against it. This subordination is what makes incoherent pairings — a trust-relationship world governed by a spatial distance metric — impossible by construction: there is no independent slot for a Layout that contradicts its World.

One boundary inside the World deserves early attention: the example above includes a *current* — an environmental condition that acts on Cells over time. Whether a dynamic condition like that is a World property or is better understood as a second mechanism participating in the Run is part of the mechanism-composition question (DEC-1), and Lab designers should expect the answer to shape World templates.

---

## **15\. World Layout (a property of the World)**

The **Layout** describes how a World's Cells are arranged and which local interactions are possible.
...
The key requirement is that a Plugin can affect only what the World's Layout and declared Connections permit.
...
**A repair also owes the human meaning, not just a diff.** Requiring a person to read Python changes to verify that a repair preserved the mechanism is a semantic leak — the exact failure §1 forbids. Every repair must therefore be accompanied by a plain-language account of what changed mechanistically ("the neighbor count now excludes the cell itself; nothing else changed"). One caveat is attached permanently: that account is generated by the same class of machinery that produced the repair, which makes it the machine grading its own homework. It is classified as **interpretation** — versioned, disputable, Reader-class — and never as evidence that the repair is faithful. The code diff remains the evidence; the semantic account is how a human decides whether to look closer.
...
- return proposed changes — including, where the World declares temporal capabilities, changes proposed to take effect at a future offset (§18.5).
...
The Reactor may support execution models beyond synchronous lockstep — discrete-event ordering, declared observation staleness, delayed effect application, deterministic interleaving. Where a World declares such capabilities, they remain Reactor-owned in every respect: the Reactor's clock is the only clock, scheduled proposals are quantized to its resolution, admitted or rejected under the same contract as any other proposal, and counted against ordinary per-participant budgets. Determinism and exact replay are not negotiable properties of any execution model the Reactor offers.

This section deliberately stops at placement. Which Worlds need which temporal capabilities, what the scheduling contract looks like, and how budgets are set are DEC-3's questions, owned by an architecture decision record and the Reactor requirements — not by this document, and never by a Plugin.
...
- the participating Plugin or Plugins (see below);  
- World, including its Layout;  
- Reactor;  
- starting conditions;  
...
**How many mechanisms participate in one Run is a named foundational decision, not a settled fact.** 2.x bound one rule to one world, and v0.1 inherited that phrasing — while every Lab example in this document quietly assumes composition: fish behavior *plus* a current *plus* a temperature gradient is at least two mechanisms, arguably three. Whether composition is expressed as multiple Plugins, as dynamic World conditions, as layered mechanism stacks, or is deliberately refused, reshapes the Plugin contract, the Reactor, provenance, and every Lab template. This document does not decide it. It records that the single-Plugin reading is 2.x inheritance (§45.11), and assigns the question to DEC-1.

A Run records enough state to support reconstruction and later analysis. **"Exact or contractually equivalent" reconstruction is a fork, not a phrase.** Bit-exact replay and contract-equivalent replay are different platforms: different storage obligations, different Reactor version discipline, different strength of evidence claims. Two downstream documents left to interpret the phrase will pick different readings within a month. The fork is assigned to DEC-2; until it is decided, documents must cite the fork rather than resolve it locally.
...
**Repeat Test** Repeat the same mechanism across different permitted starting conditions or seeds.

**Small-Change Test** Change one declared condition and determine what later evidence changes.

**Try Many Settings** Systematically vary one or more declared settings under a controlled protocol.

**World Comparison** Hold the Plugin constant while comparing World conditions or layouts.

**Plugin Comparison** Hold the World and protocol constant while comparing mechanisms.
...
The failure taxonomy (§10) and the hypothesis machinery above both lean on an undefined phrase: "required confidence." Who requires it, how it is expressed, and what a Study is permitted to claim are DEC-4's questions. This document takes one position now, as a constraint on whatever DEC-4 decides: **Study's statistical stance must be modest and plainspoken** — counts, proportions, distributions, and paired comparisons, stated in ordinary language, with what was *not* tested stated alongside what was. Significance theater, invented precision, and confidence numbers that no one can trace to a computation are all violations of §5 and §43 at once. A Study that says "19 of 20 starts produced a traveler; the one failure started inside the obstacle" has said more, more honestly, than one that reports a p-value against an unstated null.
...
Platform-neutral Reader examples — the kind the core may reasonably know about:
...
- traveler detection;  
- front speed;  
- memory or hidden-state persistence.

Lab-supplied Readers carry domain meaning the core never learns. *Illustrations only, owned entirely by their Labs:* a security Lab might define compromise-spread, privilege-reached, or reinfection Readers; an ecology Lab might define schooling-cohesion or colonization-front Readers. Their appearance here is exemplary; their definitions, vocabulary, accuracy obligations, and failure modes live in Lab papers (§29–30), and nothing about them belongs in core documents. This paragraph exists because v0.1 mixed the two lists, which is precisely the Lab leak §45.8 instructs reviewers to catch — and downstream model-written documents will reproduce whatever mixing the root document models.
...
The existing 2.x library — its rules, runs, failures, and provenance — is **carried forward into the 3.x Corpus as founding evidence.** It is not archived, not orphaned, and not treated as a separate lineage. At the time of writing it is small (tens of Runs), and a deliberate scale-up by several orders of magnitude is planned near-term work, so migration will happen against a substantially larger body than exists today.

What carries forward is decided. *How* is not: identifier mapping into 3.x namespaces, comparability across engine and Reactor versions, and which 2.x derived data is recomputed under 3.x Readers versus preserved as historical readings are DEC-6's questions. The one constraint this document imposes on DEC-6: migration must not violate §7. The 2.x histories are immutable evidence; whatever 3.x does around them, it does not rewrite them.
...
2.x demonstrates the foundation at small scale: completed Runs can be played, paused, stepped, and scrubbed through time, with render styles applied over the same immutable history. The 2.2 visualization uplift additionally *specifies* — as reviewed contract, distinct from demonstrated product — activity fields, kind-stable/state-active highlighting, trails, relief, and recurrent-structure overlays. Whether each specified style is built is a repository fact this document does not assert; the distinction between what the toy proved and what the uplift promises is preserved here deliberately, because this document's source-basis section claims to describe demonstrated behavior.

That interaction model — stored history, navigable time, styles as lenses — should be preserved as a core conceit of 3.x.
...
**Everything in this section is a candidate.** These entries exist so later visualization requirements have named concepts to accept, reshape, or reject. A downstream document may cite them as prior discussion; it must not cite them as commitments.
...
A Small-Change Test can show where later state diverges from a baseline after one controlled change, rendered as an expanding influence volume, cone, path, or field derived from actual paired Run evidence.

**This View carries the document's sharpest deception risk, and the remedy is context, never censorship.** In a strongly sensitive system, *any* flip produces an expanding divergence volume; rendering one flip's cone in isolation invites the viewer to conclude the flip was special when the system diverges identically from everything. The remedy is not a gate that refuses to render — derived analysis suppressing measured evidence is the invisible truth layer §21 forbids, and gating proposals built on differentiability are unavailable besides, since generated discrete-state Python has no derivative to inspect. The remedy is that a specific divergence is presented **against the Run's ambient sensitivity** — how divergence behaves across a sample of comparable changes — so that "this change mattered unusually" and "this system diverges from any change" are visually different statements. Uniform sensitivity is not a rendering failure; it is a finding, and the View says so. Within paired deterministic Runs, the divergence itself is honestly counterfactual — everything in the cone differs because of the change; what requires context is only the implied claim that the change was *special*.
...
For any significant visual property, the system should be able to identify its source. **The table below is an exemplar of the contract's shape, not the contract.** The normative version — complete, versioned, and testable — belongs to the Visualization requirements documents; downstream writers must extend the normative version there rather than treating this illustration as closed.
...
| Divergence volume | measured difference between paired Runs, presented with ambient-sensitivity context (§25.3) |
...
> **"Why did segmentation fail?" — 42 seconds** Show the baseline propagation, apply the network change, highlight the surviving path, show the Study comparison, then identify the shared identity connection responsible.

> **"This walker survives 19 of 20 starts" — 28 seconds** Show several Runs rapidly, then a Study summary with the one failure and the conditions that distinguish it.

> **"A one-cell change reshaped the entire front" — 35 seconds** Show baseline and Small-Change Run side-by-side, then the Influence View growing through time — with its ambient-sensitivity context (§25.3), which applies to video exactly as it applies to the interactive View.
...
What does a Cell represent? What do Connections represent? What important relationships are lost? Does the Cell state pass the computational ceiling (§13.1)?
...
What does one Reactor step mean? Is synchronous stepping meaningful? Are delayed observations or event semantics (§18.5) needed?
...
- proposed mechanism;  
- Plugin source;  
- repair history, including the semantic account of each repair (§16.5);  
...
When evidence leaves SCR — exports, reports, machine-readable bundles, downstream agentic pipelines — its origin and integrity are protected by **cryptography over the record**: content addressing, hash chains over immutable histories, and signed export manifests. Boring, standard, and correct.

What is permanently rejected is the alternative that periodically sounds clever: embedding provenance *into the experimental data itself* — watermarks, statistical residues, or steganographic signatures woven into Cell state. In a platform whose scientific premise is sensitive dependence on state, modifying state to carry metadata corrupts the experiment in order to sign it; the signature would be made of altered evidence. Additionally, no such embedding both survives arbitrary lossy transformation and constitutes cryptographic proof — those are different, largely incompatible goals. If a downstream pipeline strips the record-level provenance, the honest answer is that provenance was stripped, not that the data secretly still contains it.
...
Owns: local state-bearing participant, within the computational ceiling. Does not own: domain meaning, execution order, global behavior.

### *World*

Owns: experimental environment — Cells, Layout, Connections, conditions, visibility, starting conditions. Does not own: generated mechanism, interpretation of results.

### *Generation*

Owns: propose, write, check, test, repair (with its semantic account), deliver. Does not own: execution truth or scientific validity.

### *Plugin*

Owns: readable implementation of a local mechanism. Does not own: experimental law outside its declared capabilities — including the clock.

### *Reactor*

Owns: execution semantics and authoritative state transitions, including any declared temporal capabilities. Does not own: domain meaning or post-hoc interpretation.

### *Run*

Owns: immutable history of one exact execution. Does not own: claims of robustness or causation.

### *Study*

Owns: structured multi-Run question or hypothesis, with a modest and plainspoken inference stance. Does not own: domain truth beyond the evidence it actually collected.

### *Reader*

Owns: reproducible derived measurement or interpretation. Does not own: underlying Run history.

### *Corpus*

Owns: durable relationships among meaning, mechanisms, evidence, failures, and corrections — including the carried-forward 2.x library. Does not own: operational telemetry merely because it is stored somewhere.

### *Search*

Owns: retrieval across the Corpus. Does not own: generation or execution.

### *Visualization*

Owns: evidence-backed visual representations and time navigation. Does not own: fabricated scientific claims.

### *Lab*

Owns: domain assumptions, vocabulary, fit, accuracy, specialized Readers and Studies. Does not own: core execution semantics.

### *Platform Services*

Owns: reliable operation of the product. Does not own: the conceptual scientific model.
...
The 3.x repository should distinguish several kinds of documentation rather than allowing one omnibus to become permanent doc hell. Roughly two hundred documents are expected in this tree, most of them written by language models under human direction. That changes the failure mode: a human-written tree drifts at the leaves; a model-written tree drifts at the **root**, silently, one plausibly-interpreted ambiguity at a time. The hierarchy therefore needs not just levels but citation, precedence, and amendment discipline.

### *36.1 Document classes*

**Level 1 — Foundations.** What the system is and what major concepts mean. This document belongs here.

**Level 2 — Architecture Decisions.** Small records explaining consequential choices, alternatives, and reconsideration triggers. The Decision Registry (§40) is the index of the ones this document already knows it owes.

**Level 3 — Requirements.** Stable, testable product and subsystem contracts with permanent identifiers, following the practice proven by the 2.x omnibus: identifiers are never reused, retirements are recorded rather than silently dropped, and rationale travels with the requirement.

**Level 4 — Technical Deep Dives.** Implementation-level descriptions of each subsystem as built.

**Level 5 — Lab Papers.** Domain-specific fit, accuracy, limitations, references, benchmarks, and evidence.

**Level 6 — Operations and User Documentation.** Deployment, administration, API use, Lab operation, and human workflows.

### *36.2 Citation*

Documents cite each other by stable identifier, never by prose paraphrase alone. This document is cited as **SCR-F**, with version and section: `SCR-F v0.2 §19`. The condensed foundational rules (§44) carry permanent identifiers **F-1** through **F-22** for citation without section numbers. Decision records are cited as **DEC-n**. Requirements documents define their own permanent identifier namespaces (the 2.x omnibus's `REQ-` practice is the model) and must declare reserved identifier blocks when multiple documents write into one namespace.

### *36.3 Status lifecycle*

Every document in the tree carries exactly one status: **draft** (open to change without ceremony), **in review** (circulating for critique; the current status of this document), **adopted** (downstream documents may rely on it), **superseded** (replaced by a named successor), or **withdrawn** (retired without successor; the reason recorded). Model-written documents enter as drafts and cannot self-promote; adoption is a human act.
...
3. For **testable contract details**, the owning Requirements document outranks Foundations; if honoring a requirement would violate a foundational principle, that is not a local judgment call — it triggers the amendment procedure.  
4. A conflict discovered between adopted documents is a defect in the tree. It is recorded and resolved by amendment, never by a downstream writer silently choosing a side.

### *36.5 Amendment*

When any document — or any Run, Study, or Lab review — demonstrates that this Foundations document is wrong, the discovery is filed as a decision record proposing the amendment, with the evidence attached. If adopted, Foundations revs its version, records the change in its revision record, and the superseded text remains readable in history. Foundational identifiers (F-n, DEC-n) are permanent and never reused; a retired principle is marked retired, with the amendment that retired it.

Being wrong is recoverable. Being silently reinterpreted is not.

### *36.6 What machine-written documents owe this one*

A model writing into this tree must: cite the specific SCR-F sections and F-identifiers it depends on; flag, rather than resolve, any ambiguity it finds in them; refuse to answer a DEC-owned question locally; and keep Lab vocabulary out of Level 1–4 core documents. These obligations exist because a model will otherwise do the helpful thing — smooth over the ambiguity — and the tree will inherit an undecided decision as settled fact.

### *36.7 Canonical glossary*

The 2.x → 3.x renames are deliberate, and this table is their single source of truth. Downstream documents cite the glossary; they do not re-derive it.

| 3.x term | Meaning in one line | 2.x ancestor, where one exists |
| :---- | :---- | :---- |
| **Cell** | Smallest state-bearing participant, within the computational ceiling | Cell |
| **World** | The complete experimental environment, owning its Layout | grid \+ config (implicit) |
| **Layout** | The World's arrangement and permitted local interactions | 200×200 torus (fixed) |
| **Connection** | A declared possible local interaction between Cells | neighborhood (implicit) |
| **Generation** | Pipeline turning intent into a tested Plugin | Stage A / B / C pipeline |
| **Plugin** | Readable Python implementation of one local mechanism | generated rule / `step()` |
| **Reactor** | Deterministic execution authority | engine / harness |
| **Run** | One exact immutable execution | run |
| **Study** | Structured question across one or more Runs | — (new) |
| **Reader** | Versioned, reproducible reading of stored evidence | classifier, detector, analyzer (2.2/2.3 concepts) |
...
| **Visualization** | Evidence-backed views and time navigation | frontend player \+ 2.2 lenses |
...
- **one rule per world** (now DEC-1 rather than an assumption);  
- **strictly synchronous lockstep stepping** (now DEC-3 rather than an assumption);  
...
## **40\. Decisions this document owes and refuses to make by accident**

This document intentionally does not resolve every consequential choice. It does insist that consequential choices be *named*, so they are decided in decision records with evidence rather than answered accidentally during implementation — or worse, answered differently by different model-written documents that each thought the answer was obvious.

Each entry below becomes a DEC record in `04-decisions/`. Status at v0.2 is shown; "open" means no document may resolve it locally.

### *40.1 Foundational forks surfaced by this revision*

**DEC-1 — Mechanism composition.** *Open. The largest unnamed decision in v0.1.* How many mechanisms participate in one Run, and in what relation: multiple Plugins, dynamic World conditions, layered stacks, or a deliberate refusal to compose. Includes the environment-as-mechanism boundary (§14). Reshapes the Plugin contract, Reactor, provenance, and every Lab template. The single-Plugin reading is recorded as 2.x inheritance, not a decision.

**DEC-2 — Replay equivalence.** *Open.* Bit-exact replay versus contractually equivalent replay: which the platform promises, for which evidence classes, with what storage and Reactor-versioning obligations. Until decided, "exact or contractually equivalent" is cited as a fork, not resolved locally.

**DEC-3 — Temporal semantics.** *Open; placement decided.* Which execution models the Reactor offers beyond lockstep, which Worlds may declare them, what the scheduling contract and budgets are. The placement is already foundational law (§6, §18.5): capabilities are World/Reactor-owned, Plugins propose and never own the clock, and determinism and exact replay are non-negotiable under every model offered.

**DEC-4 — Study inference discipline.** *Open; stance constrained.* What "required confidence" means, who requires it, and what a Study may claim. Constrained by §20.3: modest, plainspoken statistics; no significance theater.

**DEC-5 — The home of semantic translation.** *Open.* Translating World descriptions, Study questions, Search queries, and repair explanations is one class of capability, currently implied across four components. Which component owns it — or whether it is a platform service with a single contract — determines where its documents live and who is accountable for its failures.

**DEC-6 — 2.x corpus migration.** *Partially decided.* Decided: the 2.x library is carried forward as founding evidence (§22.1). Open: identifier mapping, cross-version comparability, and which derived data is recomputed under 3.x Readers versus preserved as historical readings. Constrained by §7: migration never rewrites 2.x histories.

### *40.2 Open questions carried forward from v0.1*

**DEC-7 — Plugin contract surface.** What exact Python surface is expressive enough for Grid, Network, Identity, and Agent Worlds while remaining readable and enforceable.

**DEC-8 — World storage.** What common representation handles spatial and relational Worlds without forcing one into the other's shape.

**DEC-9 — Study planner autonomy.** How much SCR infers automatically from a semantic question before requiring human confirmation.

**DEC-10 — Reader trust presentation.** How Readers expose uncertainty and known failure cases in ordinary UI language.

**DEC-11 — Corpus identity.** How Same-Mechanism Families are recognized without hiding meaningful implementation differences.

**DEC-12 — Search similarity separation.** How intent similarity, mechanism similarity, and observed-behavior similarity are kept distinct.

**DEC-13 — Visualization scale.** What evidence formats advanced 3D Views require so they do not need entire Runs replayed server-side.

**DEC-14 — Video provenance.** What provenance attaches to generated narration and edits so a short video can never outrun the evidence.

**DEC-15 — Lab governance.** What minimum evidence is required before a Lab is described as validated rather than experimental.

**DEC-16 — Security isolation.** What production execution boundary replaces the hardened single-user 2.x host assumption.

**DEC-17 — Model independence.** Which parts of Generation are provider-neutral, and how model changes appear in provenance.

**DEC-18 — Human Plugin edits.** When a human directly modifies Plugin Python, how intent and provenance update so the semantic chain remains honest.

**DEC-19 — Live work.** If future Labs need live streams, how provisional live observations are separated from finalized immutable Runs.

**DEC-20 — External calibration.** How SCR packages candidate mechanisms for serious domain tools without implying domain validity it has not earned.
...
- machine explanations of machine actions — including repair accounts (§16.5) — are interpretation, never evidence.
...
Permanent identifiers; cite as F-n. Never reused; retirement requires amendment (§36.5).

1. **F-1 — Semantic first.** Humans work primarily through meaning.  
2. **F-2 — Readable and writable, always.** Generated mechanisms remain inspectable Python.  
3. **F-3 — Automation is not the objective. Better allocation of work is the objective.**  
4. **F-4 — Humans do judgment work. Machines absorb mechanical work.**  
5. **F-5 — Plain language is a system property.**  
6. **F-6 — Intent, implementation, and outcome remain separate.**  
7. **F-7 — Cells are the basic state-bearing participant, under a computational ceiling.**  
8. **F-8 — Worlds define experimental reality and own their Layout.**  
9. **F-9 — The Plugin proposes. The Reactor decides. Time is not an exception.**  
10. **F-10 — Runs are immutable evidence.**  
...
16. **Registry completeness.** Which consequential choice is still being made by accident — present in the document's assumptions but absent from the Decision Registry?  
17. **Document-system realism.** Will the citation, precedence, and amendment rules in §36 actually survive two hundred mostly machine-written documents, or is there a drift path they fail to close?

Reviewers are encouraged to propose deletions and boundary changes, not merely additions. A reviewer who resolves a DEC-owned question inline, rather than critiquing its framing, has answered a question this document deliberately left open — flag the framing instead.
...
8. **Frontend Visualization Uplift — Omnibus 2.2 (revised).** Specifies recurrent-structure detection, background declaration, activity and hidden-state views, and the observation/classification firewall. Treated as specified contract, distinct from demonstrated product (§24).  
9. **Analysis Layer and Research Lenses — Uplift 2.3 (stashed).** Holds deferred analysis-layer, probe, study, and lens-disagreement scope. Several of its concepts reappear here generalized: the analyzer concept matures into Reader, probes and studies mature into Study patterns, and §25.3's ambient-sensitivity context descends from its perturbation honesty requirements.  
10. **A Card Catalog for Emergence — position paper v0.1.** Provides the prior mechanism-hypothesis framing, Corpus/Search argument, and the explicit non-claim that retrieved mechanisms are domain predictions.  
11. **External review of v0.1 (third-party model, 2026-08).** Prompted the temporal-semantics placement (§6, §18.5), the semantic repair account (§16.5), the computational ceiling (§13.1), and — by rejection — the evidence-integrity principle (§32.1) and the influence-context requirement (§25.3). The revision record credits and disposes of each item.
