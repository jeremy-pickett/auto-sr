...
- **Major version numbers denote new products, not this app's evolution.** "3.0" (and any future whole-number bump) means a distinct, ground-up rewrite — never an increment on the current app. **And a new major supersedes everything before it.** The prior major becomes the prototype: a new major either re-examines literally everything (the 2.x→3.x documentation effort did exactly this) or spot-patches deliberately — but the old major never governs the new by default. Pulling 2.x material into 3.x is salvage, decided case by case, never inheritance. (Documents governed by the 3.x tree's own lifecycle — SCR-F's adoption machinery — keep that machinery; this rule is about the product line and working drafts.)
- **The manifesto series: latest draft supersedes all previous on sight.** v0.6 is current (repo `README.md`, canonical copy at `documents/v3_docs/scr-manifesto-v0.6.md`); v0.4 and the first README version are superseded and kept readable. This is unrelated to the omnibus requirements spec's own versioning (`asr-requirements-v3.md`, and uplift documents like `frontend-vis-uplift-2.2.1.md` that fold into a future v4) — that numbering tracks the contract, not the product. Don't conflate the two.
